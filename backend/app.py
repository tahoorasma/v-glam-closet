...
def apply_sunglasses(image, sunglasses, landmarks):
    ...
    overlay_image_alpha_sg(image, rotated_sunglasses, (top_left_y, top_left_x))
...
def overlay_image_alpha_sg(background, overlay, position):
    x, y = position
    h, w = overlay.shape[:2]
    ...
    while True:
        ...
        if faces:
            ...
            if selected_foundation:
                landmarks = shape_predictor2(gray_frame, faces[0])
                landmarks_array = np.array([[p.x, p.y] for p in landmarks.parts()])
